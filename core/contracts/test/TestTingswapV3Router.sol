...
contract TestTingswapV3Router is ITingswapV3SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        ITingswapV3Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function tingswapV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? ITingswapV3Pool(msg.sender).token0() : ITingswapV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == ITingswapV3Pool(pools[0]).token1();
            ITingswapV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(ITingswapV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(ITingswapV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
